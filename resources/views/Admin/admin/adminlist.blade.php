...
<div class="panel admin-panel">
    ...
    <table class="table table-hover text-center">
        ...
        @foreach($data as $v)
            <tr>
                <td style="text-align:left; padding-left:20px;"><input type="checkbox" name="ids" value="{{$v->user_id}}" />
                    </td>
                <td>{{$v->user_name}}</td>
                <td>{{$v->user_phone}}</td>
                <td>{{$v->user_email}}</td>
                <td>{{date("Y-m-d",$v->create_time)}}</td>
                <td><div class="button-group"><a class="button border-main" href="{{url('/role/allotrole')}}?userid={{$v->user_id}}"><span class="icon-edit (alias)"></span> 分配角色</a> <a class="button border-red" href="javascript:void(0)" onclick="Competence_del(this,'{{$v->user_id}}')"><span class="icon-trash-o"></span> 删除</a> </div></td>
            </tr>
        @endforeach
            <tr>
            ...
            <tr>
                <td colspan="8"><div class="pagelist"> <a href="{{$data->previousPageUrl()}}">上一页</a>
                        ...
                        @for($i=1;$i<=($data->lastPage());$i++)
                            <a href="http://www.home.com/admin/adminlist?page={{$i}}">{{$i}}</a>
                        @endfor
...
<div id="add_administrator_style" class="add_menber" style="display:none">
    <form action="{{url('/admin/addadmin')}}" method="post" id="form-admin-add">
